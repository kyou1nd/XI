...
function applyTheme(theme,save=true){
  const t=["dark","blue"].includes(theme)?theme:"blue";
  document.body.classList.remove("theme-bright","theme-dark","theme-blue");
  document.body.classList.add("theme-"+t);
  document.body.classList.toggle("dark",t==="dark");
  if(themeButton) themeButton.textContent=t==="dark"?"☾":"◐";
  themeOptions.forEach(b=>b.classList.toggle("active",b.dataset.theme===t));
  if(save)localStorage.setItem("xi-theme",t);
}
applyTheme(localStorage.getItem("xi-theme")==="bright"?"blue":(localStorage.getItem("xi-theme")||"blue"),false);
if(themeButton){
 themeButton.addEventListener("click",(e)=>{e.stopPropagation(); if(themeMenu){themeMenu.hidden=!themeMenu.hidden;themeButton.setAttribute("aria-expanded",String(!themeMenu.hidden));}});
 themeOptions.forEach(b=>b.addEventListener("click",()=>{applyTheme(b.dataset.theme);themeMenu.hidden=true;themeButton.setAttribute("aria-expanded","false");}));
 document.addEventListener("click",e=>{if(themeMenu&&!themeMenu.hidden&&!e.target.closest(".theme-picker-wrap")){themeMenu.hidden=true;themeButton.setAttribute("aria-expanded","false");}});
}
...
function adminRender(tab='overview'){
 const c=document.getElementById('adminContent');if(!c)return;const cfg=getSiteConfig();let storedCash={};try{storedCash=JSON.parse(localStorage.getItem('xi-class-finance-v3')||'{}')}catch{}const cash=Object.assign({balance:0,income:0,expense:0,note:'Data kas kelas'},cfg.cash||{},storedCash||{});
 if(tab==='overview')c.innerHTML=`<div class="admin-cards"><div class="admin-mini"><small>Siswa</small><b>${window.__CLASS_ACCOUNTS?.length||36}</b><span>anggota kelas</span></div><div class="admin-mini"><small>Mata Pelajaran</small><b>14</b><span>jadwal kelas</span></div><div class="admin-mini"><small>Saldo kas</small><b>${money(cash.balance)}</b><span>tersimpan lokal</span></div></div><h3 style="margin-top:25px">Kontrol Website</h3><p class="admin-help">Semua perubahan di panel ini disimpan di browser perangkat. Tidak membutuhkan server, bracket, API, atau manifest.</p><div class="task-actions"><button class="btn btn-primary" onclick="adminRender('website')">Atur Website</button><button class="mini-btn" onclick="adminRender('finance')">Atur Kas</button><button class="mini-btn" onclick="adminRender('schedule')">Atur Jadwal</button><button class="mini-btn" onclick="exportClassBackup()">Export backup</button></div>`;
  ...
  const accounts=window.__CLASS_ACCOUNTS||[], now=new Date(), tuesdays=getClassTuesdayDates(now.getFullYear(),now.getMonth()), currentTuesday=tuesdays.filter(x=>x<=now).pop();
  const monthLabel=new Intl.DateTimeFormat('id-ID',{month:'long',year:'numeric'}).format(now);
  const monthly=Array.isArray(cash.monthly)?cash.monthly:[];
  const currentMonth=monthly.find(m=>String(m.month).toLowerCase()===monthLabel.toLowerCase())||{month:monthLabel,income:0,expense:0};
  c.innerHTML=`<div class="admin-finance-head"><div><span class="eyebrow">CLASS FINANCE • XI TKJ 1</span><h3>Keuangan <em>Kelas</em></h3><p class="admin-help">Kelola saldo, pemasukan, pengeluaran, grafik bulanan, dan status iuran setiap siswa. Iuran rutin Rp 2.000 setiap hari Selasa.</p></div><div class="admin-finance-badge"><b>Rp 2.000</b><small>SETIAP SELASA</small></div></div>
  <div class="admin-finance-stats"><article><small>SALDO KAS</small><b>${money(cash.balance)}</b><span>Saldo berjalan</span></article><article><small>PEMASUKAN</small><b>${money(cash.income)}</b><span>Total pemasukan</span></article><article><small>PENGELUARAN</small><b>${money(cash.expense)}</b><span>Total pengeluaran</span></article><article><small>SELASA BERJALAN</small><b>${currentTuesday?new Intl.DateTimeFormat('id-ID',{day:'2-digit',month:'short'}).format(currentTuesday):'—'}</b><span>${tuesdays.filter(x=>x<=now).length} kewajiban bulan ini</span></article></div>
  <section class="admin-finance-editor"><div class="admin-card-head premium"><div><span>RINGKASAN KAS</span><h4>Atur data keuangan</h4></div><span class="admin-finance-month">${escapeHTML(monthLabel)}</span></div><div class="admin-form-grid"><label>Saldo kas<input type="number" id="cashBal" value="${Number(cash.balance||0)}"></label><label>Total pemasukan<input type="number" id="cashInc" value="${Number(cash.income||0)}"></label><label>Total pengeluaran<input type="number" id="cashExp" value="${Number(cash.expense||0)}"></label><label>Catatan kas<input id="cashNote" value="${escapeHTML(cash.note||'Data kas kelas')}"></label></div><div class="admin-save-row"><button class="btn btn-primary" id="saveFinanceCfg">Simpan Keuangan</button></div></section>
  <section class="admin-finance-monthly"><div class="admin-card-head premium"><div><span>MONTHLY CASH FLOW</span><h4>Grafik bulan berjalan</h4></div><div><b>${money(currentMonth.income)}</b> masuk · <b>${money(currentMonth.expense)}</b> keluar</div></div><div class="admin-finance-bars"><div><span>Pemasukan</span><i style="width:${Math.min(100,Math.max(0,Number(currentMonth.income||0)/(Math.max(1,Number(currentMonth.income||0),Number(currentMonth.expense||0)))*100))}%"></i></div><div><span>Pengeluaran</span><i style="width:${Math.min(100,Math.max(0,Number(currentMonth.expense||0)/(Math.max(1,Number(currentMonth.income||0),Number(currentMonth.expense||0)))*100))}%"></i></div></div></section>
  <section class="admin-finance-payments"><div class="admin-card-head premium"><div><span>PAYMENT ROSTER</span><h4>Status iuran siswa</h4><small>Setiap Selasa = Rp 2.000. Admin dapat mencatat pembayaran untuk tanggal hari ini.</small></div><strong>${accounts.length} siswa</strong></div><div class="admin-payment-list">${accounts.map(x=>{let d={};try{d=JSON.parse(localStorage.getItem('xi-cash-payments-'+x.nisn)||'{}')}catch{};const key=currentTuesday?localDateKey(currentTuesday):'',paid=!!(key&&d[key]);return `<article class="admin-payment-row ${paid?'paid':''}"><div class="admin-payment-avatar">${escapeHTML((x.first||x.name||'S').slice(0,1).toUpperCase())}</div><div class="admin-account-main"><b>${escapeHTML(x.name)}</b><small>${paid?'SUDAH BAYAR':'BELUM BAYAR'} ${key?'• Selasa '+new Intl.DateTimeFormat('id-ID',{day:'2-digit',month:'short'}).format(currentTuesday):''}</small></div><button class="mini-btn ${paid?'':'danger'}" type="button" data-mark-cash="${escapeHTML(x.nisn)}">${paid?'✓ Sudah bayar':'Catat Rp 2.000'}</button></article>`}).join('')||'<div class="empty-state">Data siswa belum tersedia.</div>'}</div></section>`;
 }
 else if(tab==='voting'){const ideas=window.loadClassIdeasAdmin?window.loadClassIdeasAdmin():[];c.innerHTML=`<div class="admin-head-row"><div><span class="eyebrow">CLASS VOTING</span><h3>Vooting Class</h3><p class="admin-help">Hanya Administrator yang dapat menambahkan atau menghapus usulan. Siswa hanya dapat memberikan vote.</p></div><button class="btn btn-primary" id="adminAddIdea">＋ Tambah Usulan</button></div><div class="admin-voting-list">${ideas.length?ideas.map(x=>`<article class="admin-voting-row"><div><b>${escapeHTML(x.title)}</b><small>${escapeHTML(x.author||'Admin')} • ${escapeHTML(x.date||'')} • ${Number(x.votes||0)} vote</small><p>${escapeHTML(x.desc||'')}</p></div><button class="mini-btn danger" data-delete-idea="${escapeHTML(x.id)}">Hapus</button></article>`).join(''):'<div class="empty-state">Belum ada usulan. Tambahkan usulan pertama dari Admin.</div>'}</div>`;document.getElementById('adminAddIdea')?.addEventListener('click',adminAddClassIdea);c.querySelectorAll('[data-delete-idea]').forEach(b=>b.addEventListener('click',()=>{window.deleteClassIdea?.(b.dataset.deleteIdea);adminRender('voting')}));}
 else if(tab==='song-requests'){
  c.innerHTML=`<div class="admin-head-row"><div><span class="eyebrow">MUSIC HUB</span><h3>Request Lagu</h3><p class="admin-help">Request dari siswa disimpan sebagai gambar di Google Drive. Tekan refresh untuk mengambil data terbaru.</p></div><button class="mini-btn" id="refreshSongRequests">↻ Refresh</button></div><div id="songRequestAdminList" class="admin-voting-list"><div class="empty-state">Memuat request...</div></div>`;
  const renderReq=(data)=>{const list=document.getElementById('songRequestAdminList');const rows=data?.requests||[];if(!rows.length){list.innerHTML='<div class="empty-state">Belum ada request lagu.</div>';return}list.innerHTML=rows.map(r=>`<article class="admin-voting-row"><div><b>${escapeHTML(r.title||'Tanpa judul')}</b><small>${escapeHTML(r.artist||'')} • ${escapeHTML(r.name||'Siswa')} • ${escapeHTML(r.date||'')} ${escapeHTML(r.time||'')}</small><p>NISN: ${escapeHTML(r.nisn||'-')}</p></div><a class="mini-btn" href="${escapeHTML(r.url||'#')}" target="_blank" rel="noopener">Buka Drive</a></article>`).join('')};
  const loadReq=()=>{if(!DRIVE_UPLOAD_URL){renderReq({requests:[]});return}const cb='xiSongRequests_'+Date.now();window[cb]=d=>{delete window[cb];renderReq(d)};const sc=document.createElement('script');sc.src=DRIVE_UPLOAD_URL+(DRIVE_UPLOAD_URL.includes('?')?'&':'?')+'action=listSongRequests&callback='+cb;sc.onerror=()=>{delete window[cb];renderReq({requests:[]})};document.body.appendChild(sc)};
  document.getElementById('refreshSongRequests')?.addEventListener('click',loadReq);loadReq();
 }
...
function bindAdminConfigButtons(){
 ...
 document.getElementById('saveWebsiteCfg')?.addEventListener('click',()=>{const cfg=getSiteConfig();cfg.welcome=document.getElementById('cfgWelcome').value;cfg.heroTitle=document.getElementById('cfgTitle').value;cfg.heroDescription=document.getElementById('cfgDesc').value;saveSiteConfig(cfg);applyMaintenance();applySiteConfig();toast('Website diperbarui')});
 document.getElementById('saveFinanceCfg')?.addEventListener('click',()=>{const cfg=getSiteConfig(),next={balance:+document.getElementById('cashBal').value||0,income:+document.getElementById('cashInc').value||0,expense:+document.getElementById('cashExp').value||0,note:document.getElementById('cashNote').value};cfg.cash=next;saveSiteConfig(cfg);let fin={};try{fin=JSON.parse(localStorage.getItem('xi-class-finance-v3')||'{}')}catch{};fin=Object.assign({monthly:[],transactions:[]},fin,next);localStorage.setItem('xi-class-finance-v3',JSON.stringify(fin));applySiteConfig();['cashBalance','cashIncome','cashExpense'].forEach((id,i)=>{const el=document.getElementById(id);if(el)el.textContent=money([next.balance,next.income,next.expense][i])});toast('Keuangan kelas berhasil disimpan'); exportFinanceSnapshotToDrive().then(()=>toast('Snapshot keuangan juga dikirim ke Google Drive.')).catch(()=>{}); adminRender('finance')});
 adminContent?.querySelectorAll('[data-mark-cash]').forEach(btn=>btn.addEventListener('click',()=>{const nisn=btn.dataset.markCash,d=new Date(),key=d.getFullYear()+'-'+String(d.getMonth()+1).padStart(2,'0')+'-'+String(d.getDate()).padStart(2,'0'),storage='xi-cash-payments-'+nisn;let x={};try{x=JSON.parse(localStorage.getItem(storage)||'{}')}catch{};if(x[key])return toast('Pembayaran hari ini sudah dicatat.');x[key]=true;localStorage.setItem(storage,JSON.stringify(x));let fin={};try{fin=JSON.parse(localStorage.getItem('xi-class-finance-v3')||'{}')}catch{};fin=Object.assign({balance:0,income:0,expense:0,monthly:[],transactions:[]},fin);fin.balance=Number(fin.balance||0)+2000;fin.income=Number(fin.income||0)+2000;const label=new Intl.DateTimeFormat('id-ID',{month:'long',year:'numeric'}).format(d);let m=(fin.monthly||[]).find(v=>String(v.month).toLowerCase()===label.toLowerCase());if(m)m.income=Number(m.income||0)+2000;else(fin.monthly||[]).push({month:label,income:2000,expense:0});fin.transactions=[{title:'Iuran '+(window.__CLASS_ACCOUNTS||[]).find(a=>a.nisn===nisn)?.name||'Siswa',amount:2000,type:'in',date:key},...(fin.transactions||[])].slice(0,100);localStorage.setItem('xi-class-finance-v3',JSON.stringify(fin));adminRender('finance');exportFinanceSnapshotToDrive().catch(()=>{});toast('Pembayaran Rp 2.000 dicatat.')}));
 ...
 adminContent?.querySelectorAll('[data-save-account]').forEach(btn=>btn.addEventListener('click',()=>{const row=btn.closest('[data-account-row]');adminSaveAccount(btn.dataset.saveAccount,row);adminRender('accounts')}));
...
(()=>{
 ...
 window.__getXITKJ1Session=()=>getSession();
 const setSession=u=>{
 ...
 function openAccount(){
  // Always decide the visible account surface from the CURRENT session.
  // This prevents the login panel from sitting above/behind the profile after login.
  renderAccount();
  const u=getSession();
  if(guest){guest.hidden=!!u;guest.setAttribute('aria-hidden',u?'true':'false');}
  if(profile){profile.hidden=!u;profile.setAttribute('aria-hidden',u?'false':'true');}
  if(!modal)return;
  modal.classList.add('show');
  modal.classList.toggle('showing-profile',!!u);
 ...
 function closeAccount(){
  if(!modal)return;
  modal.classList.remove('show','showing-profile');
...
function getLocalSongs(){return Array.isArray(window.LOCAL_MUSIC)?window.LOCAL_MUSIC:[]}
function songById(id){const songs=getLocalSongs();return songs[Number(id)]||null}
function renderSongSelect(){
 const sel=document.getElementById('favoriteSongSelect'),title=document.getElementById('favoritePickerTitle'),artist=document.getElementById('favoritePickerArtist'),cover=document.getElementById('favoritePickerCover'),play=document.getElementById('profileSelectedPlay');
 if(!sel)return;
 const songs=getLocalSongs();
 if(sel.value!=='' && !songs[Number(sel.value)]) sel.value='';
 const s=songs[Number(sel.value)];
 if(title)title.textContent=s?(s.name||'Lagu favorit'):'Belum memilih lagu';
 if(artist)artist.textContent=s?(s.artist||'Musik lokal'):'Pilih salah satu lagu di atas';
 if(cover){if(s?.cover){cover.src=s.cover;cover.hidden=false;}else{cover.removeAttribute('src');cover.hidden=true;}}
 const coverIcon=document.querySelector('#profileSelectedSong .profile-selected-icon');if(coverIcon)coverIcon.style.display=s?.cover?'none':'grid';
 if(play){play.disabled=!s;play.onclick=s?()=>window.playLocalSong(Number(sel.value)):null;}
 renderFavoriteInline(document.getElementById('favoriteInlineSearch')?.value||'');
}
function renderFavoriteInline(q=''){
 const list=document.getElementById('favoriteInlineList'); if(!list)return;
 const term=(q||'').trim().toLowerCase();
 const active=String(document.getElementById('favoriteSongSelect')?.value||'');
 const songs=getLocalSongs().map((s,i)=>({s,i})).filter(x=>!term||`${x.s.name||''} ${x.s.artist||''}`.toLowerCase().includes(term));
 list.innerHTML=songs.length?songs.map(({s,i})=>`<button type="button" class="favorite-inline-song ${String(i)===active?'selected':''}" data-song-id="${i}"><span class="favorite-inline-cover">${s.cover?`<img src="${escapeHTML(s.cover)}" alt="">`:'♫'}</span><span class="favorite-inline-copy"><b>${escapeHTML(s.name||('Lagu '+(i+1)))}</b><small>${escapeHTML(s.artist||'Musik lokal')}</small></span><span class="favorite-inline-state">${String(i)===active?'✓':'+'}</span></button>`).join(''):'<div class="favorite-inline-empty">Lagu tidak ditemukan.</div>';
 list.querySelectorAll('.favorite-inline-song').forEach(row=>row.addEventListener('click',()=>{const sel=document.getElementById('favoriteSongSelect');if(sel)sel.value=row.dataset.songId;renderSongSelect();}));
}
function renderFavoritePicker(q=''){renderFavoriteInline(q)}
function openFavoritePicker(){document.getElementById('favoriteInlineSearch')?.focus();renderFavoriteInline(document.getElementById('favoriteInlineSearch')?.value||'')}
function closeFavoritePicker(){const m=document.getElementById('favoritePickerModal');if(m){m.classList.remove('show');m.setAttribute('aria-hidden','true')}}

function getMusicPlayerOpen(){return document.getElementById('musicPanel')}
window.playLocalSong=function(id){const open=window.__openMusicPlayer; if(typeof open==='function')open(); window.setTimeout(()=>{const row=document.querySelector(`.music-row[data-song="${Number(id)}"] .music-row-main`); if(row)row.click();},120)}
function renderProfileMusic(u,prof){const sel=document.getElementById('favoriteSongSelect');if(sel)sel.value=prof.favoriteSongId===''?'':String(prof.favoriteSongId);renderSongSelect();const editor=document.getElementById('profilePlaylistEditor');if(editor){const ids=Array.isArray(prof.playlist)?prof.playlist:[];editor.innerHTML=ids.length?ids.map((id,i)=>{const s=songById(id);return s?`<div class="profile-playlist-row"><img src="${escapeHTML(s.cover||'')}" alt=""><span><b>${escapeHTML(s.name)}</b><small>${escapeHTML(s.artist||'')}</small></span><button class="mini-btn" type="button" onclick="playLocalSong(${Number(id)})">▶</button></div>`:''}).join(''):'<small style="color:var(--muted)">Playlist pribadi masih kosong.</small>'}}
function renderStudentMusic(profile){const wrap=document.getElementById('studentProfileFavoriteSong'),top=document.getElementById('studentProfileTopSongs');if(!wrap)return;const s=songById(profile.favoriteSongId);wrap.innerHTML=s?`<div class="profile-song"><img class="profile-song-cover" src="${escapeHTML(s.cover||'')}" alt=""><div class="profile-song-info"><b>${escapeHTML(s.name)}</b><small>${escapeHTML(s.artist||'')}</small></div><div class="profile-song-actions"><button type="button" onclick="playLocalSong(${Number(profile.favoriteSongId)})">▶ Play</button></div></div>`:'<div class="profile-song"><div class="profile-song-info"><b>Belum memilih lagu</b><small>Siswa belum mengatur lagu favorit.</small></div></div>';const ids=[...(Array.isArray(profile.playlist)?profile.playlist:[])].slice(0,5);if(top)top.innerHTML=ids.length?`<small style="color:var(--muted)">Top 5 / playlist siswa</small>`+ids.map(id=>{const x=songById(id);return x?`<div class="profile-playlist-row"><img src="${escapeHTML(x.cover||'')}" alt=""><span>${escapeHTML(x.name)}<small>${escapeHTML(x.artist||'')}</small></span><button class="mini-btn" type="button" onclick="playLocalSong(${Number(id)})">▶</button></div>`:''}).join(''):'<small style="color:var(--muted)">Belum ada playlist.</small>'}
...
function renderAccount(){
 ...
 if(!u){
   guest.hidden=false;guest.setAttribute('aria-hidden','false');
   profile.hidden=true;profile.setAttribute('aria-hidden','true');
   modal?.classList.remove('showing-profile');
   updateMobileAccount(null);guestTop();return
 }
 guest.hidden=true;guest.setAttribute('aria-hidden','true');
 profile.hidden=false;profile.setAttribute('aria-hidden','false');
 modal?.classList.add('showing-profile');
 const prof=getProfile(u),isAdmin=u.role==='admin';
 renderProfileMusic(u,prof);
...
function syncAccountBio(){const u=getSession();if(!u||u.role!=='student')return;const prof=getProfile(u);const bioEl=document.getElementById('accountBio'),input=document.getElementById('accountBioInput');if(bioEl)bioEl.textContent=prof.bio||'Belum ada bio.';if(input)input.value=prof.bio||'';}
 ...
 document.getElementById('saveFavoriteSong')?.addEventListener('click',()=>{const u=getSession();if(!u||u.role!=='student')return toast('Login sebagai siswa terlebih dahulu.');const p=getProfile(u);const id=document.getElementById('favoriteSongSelect')?.value||'';p.favoriteSongId=id;if(id!==''&&!Array.isArray(p.playlist))p.playlist=[];if(id!==''&&!p.playlist.includes(Number(id)))p.playlist.unshift(Number(id));p.playlist=[...new Set(p.playlist)].slice(0,5);saveProfile(u,p);renderAccount();renderFavoritePicker();renderPlaylistClassTKJ1();profileToast(id!==''?'Lagu favorit berhasil disimpan.':'Lagu favorit dihapus.');});
 document.getElementById('openFavoritePicker')?.addEventListener('click',openFavoritePicker);
 document.getElementById('favoritePickerClose')?.addEventListener('click',closeFavoritePicker);
 document.getElementById('favoritePickerModal')?.addEventListener('click',e=>{if(e.target.id==='favoritePickerModal')closeFavoritePicker()});
 document.getElementById('favoritePickerSearch')?.addEventListener('input',e=>renderFavoritePicker(e.target.value));
 document.getElementById('favoriteInlineSearch')?.addEventListener('input',e=>renderFavoriteInline(e.target.value));
 renderFavoriteInline();
function renderPlaylistClassTKJ1(){
 const list=document.getElementById('classRadioList'),now=document.getElementById('classRadioNow'),count=document.getElementById('playlistTrackCount'); if(!list)return;
 const all=[];Object.values(getProfiles()).forEach(p=>{if(Array.isArray(p.playlist))p.playlist.forEach(id=>{const n=Number(id);if(Number.isFinite(n)&&!all.some(x=>x.id===n))all.push({id:n})})});
 const songs=all.map(x=>({id:x.id,s:songById(x.id)})).filter(x=>x.s);
 if(count)count.textContent=`${songs.length} lagu${songs.length===1?'':'s'}`;
 const current=songById(songs[0]?.id);
 if(now)now.innerHTML=current?`<div class="playlist-now-art"><img src="${escapeHTML(current.cover||'')}" alt=""><span>♪</span></div><div class="playlist-now-copy"><span>FEATURED TRACK</span><b>${escapeHTML(current.name)}</b><small>${escapeHTML(current.artist||'Musik lokal')} · Dipilih siswa XI TKJ 1</small></div><button class="playlist-play-main" type="button" aria-label="Putar ${escapeHTML(current.name)}" onclick="playLocalSong(${songs[0].id})">▶</button>`:`<div class="playlist-empty"><span>♫</span><div><b>Belum ada lagu di playlist kelas</b><small>Pilih lagu favorit dari profil siswa untuk mengisi playlist bersama.</small></div></div>`;
 list.innerHTML=songs.length?songs.slice(0,15).map((x,i)=>`<button type="button" class="playlist-track" onclick="playLocalSong(${x.id})"><span class="playlist-track-number">${String(i+1).padStart(2,'0')}</span><span class="playlist-track-art"><img src="${escapeHTML(x.s.cover||'')}" alt=""><i>▶</i></span><span class="playlist-track-info"><b>${escapeHTML(x.s.name)}</b><small>${escapeHTML(x.s.artist||'Musik lokal')}</small></span><span class="playlist-track-arrow">›</span></button>`).join(''):'<div class="playlist-empty-list"><b>Playlist masih kosong</b><small>Belum ada siswa yang menambahkan lagu favorit.</small></div>';
}
renderPlaylistClassTKJ1();
document.getElementById('addPlaylistClassTKJ1Btn')?.addEventListener('click',()=>{const u=getSession();if(!u||u.role!=='student')return toast('Login sebagai siswa untuk menambahkan lagu.');const p=getProfile(u);if(p.favoriteSongId==='')return toast('Pilih lagu favorit di Profile terlebih dahulu.');if(!p.playlist.includes(Number(p.favoriteSongId)))p.playlist.unshift(Number(p.favoriteSongId));p.playlist=[...new Set(p.playlist)].slice(0,5);saveProfile(u,p);renderPlaylistClassTKJ1();toast('Lagu ditambahkan ke Playlist Class TKJ1.');});
document.getElementById('openMusicFromPlaylist')?.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();if(typeof window.__openMusicPlayer==='function')window.__openMusicPlayer();});
...
/* ===== HARDENED MUSIC BUTTON FALLBACKS =====
   Keep these independent from initialization order so mobile taps always work. */
document.addEventListener('click',e=>{
  const music=e.target.closest?.('#musicButton');
  const playlist=e.target.closest?.('#openMusicFromPlaylist');
  if((music||playlist) && typeof window.__openMusicPlayer==='function'){
    e.preventDefault();
    e.stopPropagation();
...
(()=>{
 ...
 const getSongs=()=> (Array.isArray(window.LOCAL_MUSIC)?window.LOCAL_MUSIC:[]).map((s,i)=>({id:i,name:s.name||'Lagu '+(i+1),artist:s.artist||'',src:s.src||'',cover:s.cover||'',type:s.type||'audio/mpeg'}));
 let currentId=null;
 ...
 audio?.addEventListener('play',updateRows);
 audio?.addEventListener('pause',updateRows);
 ...
 audio?.addEventListener('error',()=>{
   try{
     ...
     const i=Number(audio.dataset.sourceIndex||0);
     if(i+1<sources.length){
       audio.dataset.sourceIndex=String(i+1);
       audio.src=sources[i+1];
       audio.load();
       const p=audio.play();
   ...
   showMusicError('Lagu belum bisa diputar dari Google Drive. Pastikan file MP3 dibagikan sebagai “Anyone with the link”.');
   if(currentId!==null){currentId=null;updateRows();}
 });
 function showMusicError(msg){
   const t=document.getElementById('toast');
   if(!t)return;
   t.className='toast music-error-toast show';
   t.textContent=msg;
   clearTimeout(window.__musicErrorToast);
   window.__musicErrorToast=setTimeout(()=>{t.classList.remove('show');t.classList.remove('music-error-toast')},3600);
 }
 render();
 window.addEventListener('xi-drive-assets-ready',()=>{currentId=null;setNowPlaying(null);render()});
})();


/* ===== MUSIC PLAYER: COMFORTABLE DRAGGING ON PHONE + DESKTOP ===== */
(()=>{
 const panel=document.getElementById('musicPanel'),handle=panel?.querySelector('[data-music-drag-handle]'); if(!panel||!handle)return;
 let dragging=false,startX=0,startY=0,startL=0,startT=0;
 const clamp=(v,min,max)=>Math.max(min,Math.min(max,v));
 function begin(e){
   if(e.target.closest('button,input,a,audio'))return;
   const p=e.touches?.[0]||e; dragging=true;
   const r=panel.getBoundingClientRect(); startX=p.clientX;startY=p.clientY;startL=r.left;startT=r.top;
 ...
 function move(e){if(!dragging)return;const p=e.touches?.[0]||e;const r=panel.getBoundingClientRect();const left=clamp(startL+p.clientX-startX,8,window.innerWidth-r.width-8);const top=clamp(startT+p.clientY-startY,58,window.innerHeight-r.height-8);panel.style.left=left+'px';panel.style.top=top+'px';e.preventDefault?.()}
 function end(){dragging=false;handle.classList.remove('dragging')}
 handle.addEventListener('pointerdown',begin,{passive:false});window.addEventListener('pointermove',move,{passive:false});window.addEventListener('pointerup',end);
 handle.addEventListener('touchstart',begin,{passive:false});window.addEventListener('touchmove',move,{passive:false});window.addEventListener('touchend',end);
 window.addEventListener('resize',()=>{if(!panel.classList.contains('show'))return;const r=panel.getBoundingClientRect();panel.style.left=clamp(r.left,8,Math.max(8,innerWidth-r.width-8))+'px';panel.style.top=clamp(r.top,58,Math.max(58,innerHeight-r.height-8))+'px'});
})();

/* ===== REQUEST LAGU POPUP → GOOGLE DRIVE IMAGE ===== */
(()=>{
 const openBtn=document.getElementById('openPlaylistRequestBtn');
 const modal=document.getElementById('requestLaguModal');
 const closeBtn=document.getElementById('requestLaguClose');
 const form=document.getElementById('playlistRequestForm');
 if(!openBtn||!modal||!form)return;
 const open=()=>{modal.classList.add('show');modal.setAttribute('aria-hidden','false');setTimeout(()=>document.getElementById('requestSongTitle')?.focus(),80)};
 ...
 form.addEventListener('submit',async e=>{
   e.preventDefault();
   const title=document.getElementById('requestSongTitle')?.value.trim(),artist=document.getElementById('requestSongArtist')?.value.trim();
   if(!title||!artist)return toast('Judul lagu dan artis/band wajib diisi.');
   const btn=form.querySelector('button[type=submit]');
   if(btn){btn.disabled=true;btn.textContent='Mengirim…'}
   try{await submitSongRequest(title,artist);form.reset();close();toast('Request lagu berhasil dikirim ke Google Drive.')}
   catch(err){toast(err?.message||'Request lagu gagal dikirim.')}
...
/* ===== PREMIUM CLASS FINANCE + IDEA BOARD ===== */
(()=>{
 const FIN='xi-class-finance-v3', IDEAS='xi-class-ideas-v1';
 const defaultFinance={balance:0,income:0,expense:0,monthly:[{month:'Juni',income:0,expense:0},{month:'Juli',income:0,expense:0},{month:'Agustus',income:0,expense:0},{month:'September',income:0,expense:0}],transactions:[]};
 function getFin(){try{const saved=JSON.parse(localStorage.getItem(FIN)||'{}'),cfg=getSiteConfig().cash||{};return Object.assign({},defaultFinance,cfg,saved)}catch{return defaultFinance}}
 function setFin(x){localStorage.setItem(FIN,JSON.stringify(x));renderFin()}
 function renderFin(){const f=getFin();const b=document.getElementById('cashBalance'),i=document.getElementById('cashIncome'),e=document.getElementById('cashExpense');if(b)b.textContent=money(f.balance);if(i)i.textContent=money(f.income);if(e)e.textContent=money(f.expense);const chart=document.getElementById('financeChart');if(chart){const max=Math.max(1,...(f.monthly||[]).flatMap(m=>[+m.income||0,+m.expense||0]));chart.innerHTML=(f.monthly||[]).map(m=>`<div class="finance-bar-row"><span>${escapeHTML(m.month)}</span><div class="finance-bars"><i style="width:${Math.round((m.income/max)*100)}%" title="Pemasukan ${money(m.income)}"></i><em style="width:${Math.round((m.expense/max)*100)}%" title="Pengeluaran ${money(m.expense)}"></em></div><small>${money((+m.income||0)- (+m.expense||0))}</small></div>`).join('')}const tx=document.getElementById('financeTransactions');if(tx)tx.innerHTML=(f.transactions||[]).slice(0,8).map(t=>`<div class="transaction"><span class="transaction-icon ${t.type==='in'?'in':'out'}">${t.type==='in'?'↗':'↘'}</span><div><b>${escapeHTML(t.title||'Transaksi')}</b><small>${escapeHTML(t.date||'')}</small></div><strong>${t.type==='in'?'+':'-'} ${money(t.amount)}</strong></div>`).join('')||'<div class="finance-empty">Belum ada transaksi tercatat.</div>';renderPersonalFinance()}
 function getTuesdayDates(year,month){const out=[];const d=new Date(year,month,1);while(d.getMonth()===month){if(d.getDay()===2)out.push(new Date(d));d.setDate(d.getDate()+1)}return out}
 function renderPersonalFinance(){const u=window.__getXITKJ1Session?window.__getXITKJ1Session():null,st=document.getElementById('personalCashStatus');if(!st)return;if(!u||u.role!=='student'){st.textContent='Login diperlukan';st.className='status-unpaid';const a=document.getElementById('personalCashThisWeek');if(a)a.textContent='—';const t=document.getElementById('personalCashTotal');if(t)t.textContent='—';return}const key='xi-cash-payments-'+u.nisn;let d={};try{d=JSON.parse(localStorage.getItem(key)||'{}')}catch{}const now=new Date(),year=now.getFullYear(),month=now.getMonth(),tuesdays=getTuesdayDates(year,month),due=tuesdays.filter(x=>x<=now).pop(),dueKey=due?due.toISOString().slice(0,10):'',paidToday=!!(dueKey&&d[dueKey]);const total=Object.entries(d).filter(([k,v])=>v&&k.startsWith(`${year}-${String(month+1).padStart(2,'0')}-`)).length;st.textContent=paidToday?'Sudah bayar':'Belum bayar';st.className=paidToday?'status-paid':'status-unpaid';const a=document.getElementById('personalCashThisWeek');if(a)a.textContent=paidToday?'Rp 2.000':'Rp 0';const t=document.getElementById('personalCashTotal');if(t)t.textContent=money(total*2000);const next=tuesdays.find(x=>x>now)||getTuesdayDates(year,month+1)[0];const n=document.getElementById('personalCashNext');if(n)n.textContent=next?new Intl.DateTimeFormat('id-ID',{day:'2-digit',month:'short'}).format(next):'Selasa';const h=document.getElementById('personalCashHistory');if(h){const entries=Object.entries(d).filter(([k,v])=>v).slice(-8).reverse();h.innerHTML=entries.length?entries.map(([k])=>`<div class="profile-playlist-row"><span>Selasa • ${escapeHTML(k)}</span><b class="status-paid">Rp 2.000 ✓</b></div>`).join(''):'<small style="color:var(--muted)">Belum ada riwayat pembayaran.</small>'}}
 window.setStudentCashPayment=function(){const u=window.__getXITKJ1Session?window.__getXITKJ1Session():null;if(!u||u.role!=='student')return toast('Login sebagai siswa terlebih dahulu.');const d=new Date();if(d.getDay()!==2)return toast('Pembayaran kas hanya dicatat pada hari Selasa.');const dKey=d.toISOString().slice(0,10),key='xi-cash-payments-'+u.nisn;let data={};try{data=JSON.parse(localStorage.getItem(key)||'{}')}catch{};data[dKey]=true;localStorage.setItem(key,JSON.stringify(data));renderPersonalFinance();toast('Pembayaran kas Rp 2.000 dicatat.')}
 renderFin();
})();
(()=>{
 const key='xi-class-ideas-v1';
 function load(){try{return JSON.parse(localStorage.getItem(key)||'[]')}catch{return[]}}
 function save(x){localStorage.setItem(key,JSON.stringify(x));render()}
 function render(){const el=document.getElementById('ideaList');if(!el)return;const a=load();el.innerHTML=a.length?a.slice().sort((x,y)=>(y.votes||0)-(x.votes||0)).map((x,i)=>`<article class="idea-item"><div class="idea-rank">${String(i+1).padStart(2,'0')}</div><button class="idea-vote mini-btn" onclick="voteIdea('${x.id}')">▲ <b>${x.votes||0}</b><small>vote</small></button><div class="idea-copy"><b>${escapeHTML(x.title)}</b><small>${escapeHTML(x.author||'Admin')} • ${escapeHTML(x.date||'')}</small><p>${escapeHTML(x.desc||'')}</p></div></article>`).join(''):'<div class="finance-empty">Belum ada usulan. Admin dapat menambahkan voting dari Control Panel.</div>'}
 window.voteIdea=id=>{const a=load(),x=a.find(v=>v.id===id);if(!x)return;x.votes=(x.votes||0)+1;save(a);toast('Vote kamu tercatat.')};
 window.loadClassIdeasAdmin=load;
 window.deleteClassIdea=id=>{const a=load().filter(x=>x.id!==id);localStorage.setItem(key,JSON.stringify(a));toast('Usulan dihapus.')};
 window.adminAddClassIdea=async()=>{const title=await openTaskInput({title:'Tambah Usulan Vooting',hint:'Masukkan judul pilihan/usulan kelas.',placeholder:'Contoh: Seragam kelas baru'});if(!title)return;const desc=await openTaskInput({title:'Deskripsi Usulan',hint:'Jelaskan pilihan atau usulan ini.',textarea:true,placeholder:'Tulis penjelasan singkat...'});const a=load();const u=getSessionSafe()||{};a.unshift({id:Date.now().toString(),title:title.trim(),desc:(desc||'').trim(),author:u.name||'Administrator',date:new Date().toLocaleDateString('id-ID'),votes:0});save(a);toast('Usulan berhasil ditambahkan ke Vooting Class.');adminRender('voting')};
 render();
})();;
...
/* ===== DRIVE IMAGE EXPORTS: KAS + REQUEST LAGU ===== */
function makeDriveCardImage_(title, rows, accent='XI TKJ 1') {
  const w=1200, rowH=92, h=Math.max(620, 270 + rows.length*rowH);
  const c=document.createElement('canvas'); c.width=w; c.height=h;
  const x=c.getContext('2d');
  const g=x.createLinearGradient(0,0,w,h); g.addColorStop(0,'#0b1025'); g.addColorStop(1,'#17102c'); x.fillStyle=g; x.fillRect(0,0,w,h);
  x.fillStyle='#6b5cff'; x.fillRect(0,0,14,h);
  x.fillStyle='#f5f6ff'; x.font='800 44px Arial'; x.fillText(title,58,78);
  x.fillStyle='#9da5c8'; x.font='500 22px Arial'; x.fillText(accent,60,118);
  let y=188;
  rows.forEach((r,i)=>{
    x.fillStyle='rgba(255,255,255,.055)'; x.fillRect(48,y-38,w-96,70);
    x.fillStyle='#9da5c8'; x.font='600 18px Arial'; x.fillText(String(r.label||''),70,y);
    x.fillStyle='#f5f6ff'; x.font='700 26px Arial'; x.fillText(String(r.value||''),360,y);
    y+=rowH;
  });
  x.fillStyle='#777f9e'; x.font='500 16px Arial'; x.fillText('Dibuat otomatis oleh XI TKJ 1 Class Website • '+new Date().toLocaleString('id-ID'),60,h-38);
  return c.toDataURL('image/png').split(',')[1];
}
function sendDriveImage_(imageData,fileName,kind,meta={}){
  if(!DRIVE_UPLOAD_URL)return Promise.reject(new Error('Google Drive belum dikonfigurasi'));
  return postDriveForm_({action:kind==='song-request'?'saveSongRequest':kind==='finance-snapshot'?'saveFinanceSnapshot':'saveDriveImage',imageData,mimeType:'image/png',fileName,kind,meta:JSON.stringify(meta),title:meta.title||'',artist:meta.artist||'',name:meta.name||'',nisn:meta.nisn||'',date:localDateKey(),time:new Date().toLocaleTimeString('id-ID',{hour12:false})});
}
function exportFinanceSnapshotToDrive(){
  const f=getFin(); const month=new Intl.DateTimeFormat('id-ID',{month:'long',year:'numeric'}).format(new Date());
  const data=makeDriveCardImage_('Keuangan Kelas',[
    {label:'SALDO KAS',value:money(f.balance)},
    {label:'PEMASUKAN',value:money(f.income)},
    {label:'PENGELUARAN',value:money(f.expense)},
    {label:'IURAN RUTIN',value:'Rp 2.000 / Selasa'},
    {label:'PERIODE',value:month}
  ]);
  return sendDriveImage_(data,'Keuangan-Kelas-'+localDateKey()+'.png','finance-snapshot',{month,balance:f.balance,income:f.income,expense:f.expense});
}
function submitSongRequest(title,artist){
  const u=getSessionSafe?.(); if(!u||u.role!=='student')return Promise.reject(new Error('Login sebagai siswa untuk mengirim request lagu.'));
  const name=u.name||u.displayName||'Siswa';
  const data=makeDriveCardImage_('Request Lagu',[
    {label:'NAMA SISWA',value:name},
    {label:'JUDUL LAGU',value:title},
    {label:'ARTIS / BAND',value:artist},
    {label:'WAKTU',value:new Date().toLocaleString('id-ID')}
  ],'XI TKJ 1 • MUSIC HUB');
  return sendDriveImage_(data,'Request-Lagu-'+cleanFileName_(name)+'-'+cleanFileName_(title)+'.png','song-request',{name,title,artist,nisn:u.nisn||''});
}
function cleanFileName_(v){return String(v||'data').replace(/[^a-z0-9 _.-]/gi,'').trim().replace(/\\s+/g,'-').slice(0,60)||'data'}
...
(()=>{
  ...
  function openStudentBarcode(){
    ...
    img.alt='Memuat barcode '+account.nisn+'…';
    (async()=>{
      const url=await (window.getDriveBarcodeUrl?.(account.nisn)||Promise.resolve(''));
      if(url){img.src=url;img.alt='Barcode NISN '+account.nisn;}
      else {img.alt='Barcode belum ditemukan di Google Drive';toast('Barcode siswa belum ditemukan di folder Barcode Siswa.');}
    })();
  }
